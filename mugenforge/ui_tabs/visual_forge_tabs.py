...
class SoundCueEditorTab(AppBackedTab):
    def __init__(self, app):
        super().__init__(app)
        self._build()
    ...
    def _build(self):
        ...
        ttk.Label(
            header,
            text='Assign and time PlaySnd cues to specific StateDefs/AnimElem frames. Inserts are text-based with backups and should be verified in M.U.G.E.N.',
            wraplength=1120,
            justify='left',
        ).grid(row=0, column=0, sticky='ew')
        left = ttk.Frame(tab, padding=(8, 0))
        left.grid(row=1, column=0, sticky='nsw')
        ttk.Button(left, text='Refresh / Build Cue Manifest', command=self.app.vf_refresh_sound_cues_ui).grid(row=0, column=0, sticky='ew', pady=2)
        ttk.Label(left, text='Target StateDef').grid(row=1, column=0, sticky='w', pady=(8, 2))
        self.app.vf_sound_state_var = tk.StringVar(value='200')
        self.app.vf_sound_state_combo = ttk.Combobox(left, textvariable=self.app.vf_sound_state_var, width=34)
        self.app.vf_sound_state_combo.grid(row=2, column=0, sticky='ew')
        fields = ttk.LabelFrame(left, text='Cue', padding=(6, 6))
        fields.grid(row=3, column=0, sticky='ew', pady=(8, 0))
        self.app.vf_sound_frame_var = tk.StringVar(value='2')
...
class MoveComposer2Tab(AppBackedTab):
    def __init__(self, app):
        super().__init__(app)
        self._build()
    ...
    def _build(self):
        tab = ttk.Frame(self.notebook)
        self.frame = tab
        self.app.vf_composer2_frame = tab
        tab.columnconfigure(1, weight=1)
        tab.rowconfigure(1, weight=1)
        ...
        ttk.Label(
            header,
            text='A no-code move builder tied to Visual Timeline, Sound Cue, and AIR/CLSN scaffolding. Preview first, then append to project files with backups.',
            wraplength=1120,
            justify='left',
        ).grid(row=0, column=0, sticky='ew')
        left = ttk.Frame(tab, padding=(8, 0))
        left.grid(row=1, column=0, sticky='nsw')
        self.app.vf_compose_vars = {}
        defaults = [
        ...
        right.columnconfigure(0, weight=1)
        right.rowconfigure(0, weight=1)
        self.app.vf_composer2_output = tk.Text(right, wrap='none', font=('Consolas', 10), state='disabled')
        y = ttk.Scrollbar(right, orient='vertical', command=self.app.vf_composer2_output.yview)
        x = ttk.Scrollbar(right, orient='horizontal', command=self.app.vf_composer2_output.xview)
        self.app.vf_composer2_output.configure(yscrollcommand=y.set, xscrollcommand=x.set)
        self.app.vf_composer2_output.grid(row=0, column=0, sticky='nsew')
        y.grid(row=0, column=1, sticky='ns')
        x.grid(row=1, column=0, sticky='ew')

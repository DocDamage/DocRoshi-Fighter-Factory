...
from ..forge_timeline import (
    FORGE_TIMELINE_VERSION,
    apply_clsn_track_sheet,
    apply_hitdef_track_sheet,
    apply_move_timeline_edit_sheet,
    apply_palette_editor_sheet,
    apply_state_graph_edit_sheet,
    build_editable_state_graph,
    build_forge_timeline_bundle,
    build_integrated_move_timeline_model,
    export_clsn_track_sheet,
    export_hitdef_track_sheet,
    export_move_timeline_edit_sheet,
    run_forge_timeline_pass,
    write_forge_timeline_dashboard,
    write_palette_studio,
    write_stage_live_preview,
)
...
class ForgeTimelineTab(AppBackedTab):
    def __init__(self, app):
        super().__init__(app)
        self.forge_timeline_preview_photo = None
        self._build()
        self._publish(
            '_forge_timeline_root',
            '_forge_timeline_action_filter',
            '_forge_timeline_show_result',
            '_forge_timeline_load_preview_image',
            'forge_timeline_one_click_ui',
            'forge_timeline_dashboard_ui',
            'forge_timeline_build_model_ui',
            'forge_timeline_export_move_sheet_ui',
            'forge_timeline_apply_move_sheet_ui',
            'forge_timeline_export_clsn_sheet_ui',
            'forge_timeline_apply_clsn_sheet_ui',
            'forge_timeline_export_hitdef_sheet_ui',
            'forge_timeline_apply_hitdef_sheet_ui',
            'forge_timeline_state_graph_ui',
            'forge_timeline_apply_state_graph_ui',
            'forge_timeline_palette_ui',
            'forge_timeline_apply_palette_ui',
            'forge_timeline_stage_preview_ui',
            'forge_timeline_bundle_ui',
        )

    def _build(self):
        frame = ttk.Frame(self.notebook)
        self.frame = frame
        self.app.forge_timeline_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)
        ...
        self.app.forge_timeline_action_var = self.forge_timeline_action_var
        ...
        self.app.forge_timeline_canvas = self.forge_timeline_canvas
        ...
        self.app.forge_timeline_help = self.forge_timeline_help
        self.forge_timeline_help.grid(row=0, column=0, sticky='nsew')
        body.add(help_frame, weight=1)
        self._set_text(self.forge_timeline_help, (
        ...
        self.app.forge_timeline_output = self.forge_timeline_output
        self.forge_timeline_output.grid(row=3, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.app.forge_timeline_preview_photo = self.forge_timeline_preview_photo
        self.notebook.add(frame, text='Forge Timeline')
        self._set_text(self.forge_timeline_output, 'Forge Timeline v4.5 is ready. Open a project, then run One-Click Timeline Pass.')
    ...
    def _forge_timeline_action_filter(self):
        raw = self.forge_timeline_action_var.get().strip()
        if not raw:
            return None
        try:
            return int(raw)
    ...
    def _forge_timeline_show_result(self, result, status: str):
        self._set_text(self.forge_timeline_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self._set_status(status)
        self._select(self.frame)

...
from .rescue_lab import (
    RESCUE_LAB_VERSION, run_rescue_lab_pass, scan_project_embedded_assets,
    build_recovery_manifest_from_air, create_rescue_contact_sheet,
    build_recovered_sff_v1, write_beginner_rescue_plan,
)
from .sff2_bridge import (
    write_sprmake2_bridge_project, run_sprmake2, export_current_sff1_to_sff2_source,
    inspect_sprmake2_def, create_sff2_source_pack_zip, write_sff2_bridge_guide,
)
...
from .forge_beyond import (
    FORGE_BEYOND_VERSION, ImageEditSpec, run_source_image_editor, import_archive_project,
...
from .forge_polish import (
    FORGE_POLISH_VERSION, run_forge_polish_pass, write_beginner_dashboard_41,
    validate_all_edit_sheets, export_editable_sound_cue_sheet, apply_snd_cue_sheet,
    build_sprite_backed_timeline_preview, write_state_graph_canvas_artifacts,
    build_state_graph_canvas_model as polish_build_state_graph_canvas_model,
    validate_template_architecture, import_template_pack as polish_import_template_pack,
    write_stage_source_art_preview, write_backup_history, list_backup_rows, restore_backup_file,
    build_forge_polish_bundle,
)
from .forge_timeline import (
    FORGE_TIMELINE_VERSION, run_forge_timeline_pass, write_forge_timeline_dashboard,
    build_integrated_move_timeline_model, export_move_timeline_edit_sheet, apply_move_timeline_edit_sheet,
    export_clsn_track_sheet, apply_clsn_track_sheet, export_hitdef_track_sheet, apply_hitdef_track_sheet,
    build_editable_state_graph, apply_state_graph_edit_sheet, write_palette_studio, apply_palette_editor_sheet,
    write_stage_live_preview, build_forge_timeline_bundle,
)

from .binary_core import (
    BINARY_CORE_VERSION, run_binary_core_pass, write_binary_core_dashboard, write_binary_safety_report,
    inspect_sff2_native, export_supported_sff2_payloads,
    export_sff_axis_sheet, apply_sff_axis_sheet, create_sff2_source_rebuild_pack, build_sff2_png_workspace_copy,
    export_snd_bank_sheet, apply_snd_bank_sheet, write_snd_waveform_preview,
    build_binary_roundtrip_report, build_binary_core_bundle,
)

from .binary_deep import (
    BINARY_DEEP_VERSION, run_binary_deep_pass, write_binary_deep_dashboard,
    inspect_sff2_deep, export_sff2_deep_sprites, export_sff2_mutation_sheet,
    apply_sff2_mutation_sheet, build_native_sff2, export_snd_patch_sheet,
    apply_snd_patch_sheet, write_runtime_lab, build_binary_deep_bundle,
)

from .binary_maturity import (
    BINARY_MATURITY_VERSION, run_binary_maturity_pass, write_sff2_maturity_report,
    export_sff2_edit_workspace, apply_sff2_edit_workspace, build_sff2_from_source_folder,
    export_snd_slot_patch_sheet, apply_snd_slot_patch_sheet,
    write_runtime_test_harness, run_runtime_test_harness, build_binary_maturity_bundle,
)
...
from .closure_lab import (
    ...
    build_sff2_corpus_validator, write_sff2_unknown_triage, export_direct_mutation_commit_sheet,
    apply_direct_mutation_commit_sheet, restore_last_mutation_snapshot, export_gameplay_tuning_sheet,
    apply_gameplay_tuning_sheet, write_report_evidence_contract, build_closure_lab_bundle,
)
from .gap_closer import (
    GAP_CLOSER_VERSION, run_gap_closer_pass, write_gap_closer_dashboard,
    probe_engine_profile, build_source_runtime_model, build_engine_scenario_sessions,
    build_engine_backed_readiness_gate, build_sff_snd_corpus_lab, write_unknown_binary_triage,
    export_binary_commit_sheet, apply_binary_commit_sheet,
    export_generated_code_tuning_lab, apply_generated_code_tuning_sheet,
    build_gap_closer_bundle,
)
...
class MugenForgeApp(tk.Tk):
    ...
    def _build_creator_hub_tab(self):
        ...
        note = tk.Text(left, wrap='word', width=48, height=12, font=('Consolas', 9), state='disabled')
        note.grid(row=3, column=0, sticky='ew', pady=(6, 0))
        self.creator_hub_note = note
        self.set_text(self.creator_hub_note, 'Best route for a non-coder: run One-Click Creator Hub Pass, open CREATOR_DASHBOARD.html, clear high-priority task cards, then use Feature Bank/Move Wizard/Sprite Lab for the fun creative changes.')
        ...
        self.set_text(self.creator_hub_output, 'Open/create a character folder, then run One-Click Creator Hub Pass. It writes beginner dashboards, cards, trials, maps, recipes, and release scaffolding without silently changing active code.')
    ...
    def _creator_hub_run(self, label, func, *args, **kwargs):
        ...
        try:
            result = func(root, *args, **kwargs)
            self.reload_project()
            self.set_text(self.creator_hub_output, result.to_text())
            self.notebook.select(self.creator_hub_frame)
            self.status_var.set(label)
        except Exception as exc:
            messagebox.showerror(label, str(exc))
    ...
    def _build_sff2_bridge_tab(self):
        ...
        note = tk.Text(left, wrap='word', width=50, height=10, font=('Consolas', 9), state='disabled')
        note.grid(row=4, column=0, sticky='ew', pady=(6, 0))
        self.sff2_bridge_note = note
        self.set_text(self.sff2_bridge_note, 'Best beginner route: name sprites like g200_i0.png → choose image folder → write Sprmake2 DEF + BAT → inspect the DEF → run the BAT with your own sprmake2.exe → update the character DEF sprite reference.')
        ...
        self.set_text(self.sff2_bridge_output, 'Open/create a character folder, then use this tab to generate a Sprmake2 source project. This is source-based SFF2 support, not arbitrary SFF2 binary patching.')

    def _sff2_bridge_root(self):
        root = self._plus_get_project_root()
        if root and hasattr(self, 'sff2_output_var') and not self.sff2_output_var.get().strip():
            self.sff2_output_var.set(f'{Path(root).name}.sff')
        return root

    def _sff2_bridge_show_result(self, result, label='SFF2 Bridge'):
        text = result.to_text() if hasattr(result, 'to_text') else str(result)
        self.set_text(self.sff2_bridge_output, text)
        if hasattr(self, 'sff2_bridge_frame'):
            self.notebook.select(self.sff2_bridge_frame)
        self.status_var.set(label)
    ...
    def sff2_bridge_export_current_ui(self):
        ...
        if not root:
            return
        sff = self._plus_find_sff_path() if hasattr(self, '_plus_find_sff_path') else None
        if not sff:
    ...
    def sff2_bridge_inspect_ui(self):
        ...
        try:
            ...
            self.set_text(self.sff2_bridge_output, text)
            self.notebook.select(self.sff2_bridge_frame)
            self.status_var.set('Sprmake2 DEF inspected')
        except Exception as exc:
    ...
    def _build_rescue_lab_tab(self):
        ...
        note = tk.Text(left, wrap='word', width=46, height=12, font=('Consolas', 9), state='disabled')
        note.grid(row=2, column=0, sticky='ew', pady=(6, 0))
        self.rescue_lab_note = note
        self.set_text(self.rescue_lab_note, 'Best beginner route: One-Click Rescue Lab Pass → open rescue_lab/RECOVERY_CONTACT_SHEET.png → replace bad/missing extracted images → Build Recovery Manifest From AIR → Build Recovered SFF v1 → test in Animation Player and CLSN Editor. This is a rescue workflow, not byte-perfect SFF v2 rebuilding.')
        ...
        self.set_text(self.rescue_lab_output, 'Open a character folder, then run One-Click Rescue Lab Pass. Review the generated beginner plan before building replacement assets.')
    ...
    def _rescue_lab_run(self, label, func, *args, **kwargs):
        ...
        try:
            result = func(root, *args, **kwargs)
            self.reload_project()
            self.set_text(self.rescue_lab_output, result.to_text())
            self.notebook.select(self.rescue_lab_frame)
            self.status_var.set(label)
        except Exception as exc:
            messagebox.showerror(label, str(exc))
    ...
    def _build_image_factory_tab(self):
        ...
        header.grid(row=0, column=0, columnspan=2, sticky='ew', padx=6, pady=6)
        ttk.Label(header, text='Batch-process source sprites without coding: transparency keying, trim, mirror, scale, pad to canvas, outline, shadow, palette reduction, and palette variant folders.', wraplength=1120, justify='left').grid(row=0, column=0, sticky='ew')

        self.image_factory_in_var = tk.StringVar(value='')
        self.image_factory_out_var = tk.StringVar(value='')
        self.image_factory_scale_var = tk.StringVar(value='100')
        self.image_factory_canvas_w_var = tk.StringVar(value='0')
        self.image_factory_canvas_h_var = tk.StringVar(value='0')
        self.image_factory_palette_var = tk.StringVar(value='0')
        self.image_factory_transparency_var = tk.StringVar(value='none')
        self.image_factory_trim_var = tk.BooleanVar(value=True)
        self.image_factory_mirror_x_var = tk.BooleanVar(value=False)
        self.image_factory_mirror_y_var = tk.BooleanVar(value=False)
        self.image_factory_outline_var = tk.BooleanVar(value=False)
        self.image_factory_shadow_var = tk.BooleanVar(value=False)
        ...
        ttk.Entry(box, textvariable=self.image_factory_in_var, width=46).grid(row=0, column=1, sticky='w')
        ttk.Label(box, text='output').grid(row=1, column=0, sticky='e')
        ttk.Entry(box, textvariable=self.image_factory_out_var, width=46).grid(row=1, column=1, sticky='w')
        ttk.Button(box, text='Choose Input Folder', command=self.image_factory_choose_input_ui).grid(row=2, column=0, columnspan=2, sticky='ew', pady=2)
        ttk.Button(box, text='Choose Output Folder', command=self.image_factory_choose_output_ui).grid(row=3, column=0, columnspan=2, sticky='ew', pady=2)

        opts = ttk.LabelFrame(left, text='Macros', padding=(6, 6))
        opts.grid(row=1, column=0, sticky='ew', pady=(6, 0))
        items = [('scale %', self.image_factory_scale_var), ('canvas w', self.image_factory_canvas_w_var), ('canvas h', self.image_factory_canvas_h_var), ('palette colors', self.image_factory_palette_var)]
        ...
        ttk.Combobox(opts, textvariable=self.image_factory_transparency_var, values=('none', 'top-left', 'magenta'), width=10, state='readonly').grid(row=len(items), column=1, sticky='w', padx=2, pady=1)
        checks = [('trim', self.image_factory_trim_var), ('mirror X', self.image_factory_mirror_x_var), ('mirror Y', self.image_factory_mirror_y_var), ('outline', self.image_factory_outline_var), ('shadow', self.image_factory_shadow_var)]
        ...
        ttk.Button(actions, text='Run Batch Image Macros', command=self.image_factory_process_ui).grid(row=0, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='Make Palette Variant Folders', command=self.image_factory_palette_variants_ui).grid(row=1, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='Use Project source_sprites → processed_sprites', command=self.image_factory_use_project_defaults_ui).grid(row=2, column=0, sticky='ew', pady=2)

        right = ttk.Frame(img, padding=(0, 0))
        right.grid(row=1, column=1, sticky='nsew', padx=(0, 6), pady=(0, 6))
        right.rowconfigure(0, weight=1)
        right.columnconfigure(0, weight=1)
        self.image_factory_output = tk.Text(right, wrap='word', font=('Consolas', 10), state='disabled')
        sy = ttk.Scrollbar(right, orient='vertical', command=self.image_factory_output.yview)
        self.image_factory_output.configure(yscrollcommand=sy.set)
        self.image_factory_output.grid(row=0, column=0, sticky='nsew')
        sy.grid(row=0, column=1, sticky='ns')
        self.notebook.add(img, text='Image Factory')
        self.set_text(self.image_factory_output, 'Choose a source folder of images. Output is always written to a separate folder so originals stay safe.')
    ...
    def _build_forge_plus_tab(self):
        ...
        ttk.Button(actions, text='Run Forge+ Doctor', command=self.forge_plus_report_ui).grid(row=0, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='Auto Setup / Repair Basics', command=self.auto_setup_project_ui).grid(row=1, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='One-Click Prototype / Rebuild Assets', command=self.auto_one_click_skeleton_ui).grid(row=2, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='Beginner Doctor Report', command=self.factory_doctor_ui).grid(row=3, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='Export Move List', command=self.forge_plus_export_move_list_ui).grid(row=4, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='Write Creator Manual', command=self.forge_plus_creator_manual_ui).grid(row=5, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='Package Character ZIP', command=self.package_character).grid(row=6, column=0, sticky='ew', pady=2)
        ttk.Button(actions, text='Refresh Project Tree', command=self.reload_project).grid(row=7, column=0, sticky='ew', pady=2)
        ...
        right.rowconfigure(0, weight=1)
        right.columnconfigure(0, weight=1)
        self.forge_plus_output = tk.Text(right, wrap='word', font=('Consolas', 10), state='disabled')
        y = ttk.Scrollbar(right, orient='vertical', command=self.forge_plus_output.yview)
        self.forge_plus_output.configure(yscrollcommand=y.set)
        self.forge_plus_output.grid(row=0, column=0, sticky='nsew')
        y.grid(row=0, column=1, sticky='ns')
        self.notebook.add(forge, text='Forge+ Doctor')
        self.set_text(self.forge_plus_output, 'Open a character folder, then run Forge+ Doctor. This gives a plain-English score and next step.')

    def _build_sprite_lab_tab(self):
        lab = ttk.Frame(self.notebook)
        self.sprite_lab_frame = lab
        ...
        self.sprite_lab_folder_var = tk.StringVar(value='')
        self.sprite_lab_group_var = tk.StringVar(value='200')
        self.sprite_lab_start_image_var = tk.StringVar(value='0')
        self.sprite_lab_canvas_w_var = tk.StringVar(value='96')
        self.sprite_lab_canvas_h_var = tk.StringVar(value='96')
        self.sprite_lab_axis_x_var = tk.StringVar(value='48')
        self.sprite_lab_axis_y_var = tk.StringVar(value='88')
        self.sprite_lab_ticks_var = tk.StringVar(value='5')
        self.sprite_lab_action_var = tk.StringVar(value='200')
        self.sprite_lab_scale_var = tk.StringVar(value='100')
        self.sprite_lab_trim_var = tk.BooleanVar(value=True)

        left = ttk.Frame(lab, padding=(6, 0))
        left.grid(row=1, column=0, sticky='nsw')
        pick = ttk.LabelFrame(left, text='Folder', padding=(6, 6))
        pick.grid(row=0, column=0, sticky='ew')
        ttk.Entry(pick, textvariable=self.sprite_lab_folder_var, width=42).grid(row=0, column=0, sticky='ew')
        ttk.Button(pick, text='Choose Image Folder', command=self.sprite_lab_choose_folder_ui).grid(row=1, column=0, sticky='ew', pady=2)
        ttk.Button(pick, text='Summarize Folder', command=self.sprite_lab_summary_ui).grid(row=2, column=0, sticky='ew', pady=2)

        fields = ttk.LabelFrame(left, text='Build settings', padding=(6, 6))
        fields.grid(row=1, column=0, sticky='ew', pady=(6, 0))
        field_items = [
            ('group', self.sprite_lab_group_var), ('start image', self.sprite_lab_start_image_var),
            ('canvas w', self.sprite_lab_canvas_w_var), ('canvas h', self.sprite_lab_canvas_h_var),
            ('axis x', self.sprite_lab_axis_x_var), ('axis y', self.sprite_lab_axis_y_var),
            ('AIR action', self.sprite_lab_action_var), ('ticks', self.sprite_lab_ticks_var),
            ('scale %', self.sprite_lab_scale_var),
        ]
        ...
        ttk.Checkbutton(fields, text='trim transparent edges', variable=self.sprite_lab_trim_var).grid(row=len(field_items), column=0, columnspan=2, sticky='w', pady=(4, 0))

        ops = ttk.LabelFrame(left, text='One-click sprite operations', padding=(6, 6))
        ops.grid(row=2, column=0, sticky='ew', pady=(6, 0))
        ttk.Button(ops, text='Make Contact Sheet', command=self.sprite_lab_contact_sheet_ui).grid(row=0, column=0, sticky='ew', pady=2)
        ttk.Button(ops, text='Normalize / Trim To Manifest', command=self.sprite_lab_normalize_ui).grid(row=1, column=0, sticky='ew', pady=2)
        ttk.Button(ops, text='Build SFF v1 From Folder', command=self.sprite_lab_build_sff_ui).grid(row=2, column=0, sticky='ew', pady=2)
        ttk.Button(ops, text='Generate AIR From Folder', command=self.sprite_lab_air_ui).grid(row=3, column=0, sticky='ew', pady=2)
        ttk.Button(ops, text='Make GIF Preview', command=self.sprite_lab_gif_ui).grid(row=4, column=0, sticky='ew', pady=2)
        ttk.Button(ops, text='Build ACT Palette From Folder', command=self.sprite_lab_palette_ui).grid(row=5, column=0, sticky='ew', pady=2)
        ttk.Button(ops, text='Convert Folder To PCX', command=self.sprite_lab_pcx_ui).grid(row=6, column=0, sticky='ew', pady=2)

        right = ttk.Frame(lab)
        right.grid(row=1, column=1, sticky='nsew', padx=(0, 6), pady=(0, 6))
        right.rowconfigure(0, weight=1)
        right.columnconfigure(0, weight=1)
        self.sprite_lab_output = tk.Text(right, wrap='word', font=('Consolas', 10), state='disabled')
        sy = ttk.Scrollbar(right, orient='vertical', command=self.sprite_lab_output.yview)
        self.sprite_lab_output.configure(yscrollcommand=sy.set)
        self.sprite_lab_output.grid(row=0, column=0, sticky='nsew')
        sy.grid(row=0, column=1, sticky='ns')
        self.notebook.add(lab, text='Sprite Lab')
        self.set_text(self.sprite_lab_output, 'Choose a folder of PNG/PCX/BMP/GIF/JPG/WEBP sprites. Use contact sheet first, then normalize/build SFF or generate AIR.')

    def _build_stage_builder_tab(self):
        stage = ttk.Frame(self.notebook)
        self.stage_builder_frame = stage
        ...
        header.grid(row=0, column=0, columnspan=2, sticky='ew', padx=6, pady=6)
        ttk.Label(header, text='Create a starter M.U.G.E.N stage from one background image. MugenForge writes the stage DEF, SFF v1, source manifest, and README.', wraplength=1100, justify='left').grid(row=0, column=0, sticky='ew')

        self.stage_image_var = tk.StringVar(value='')
        self.stage_name_var = tk.StringVar(value='new_stage')
        self.stage_out_dir_var = tk.StringVar(value='')
        self.stage_zoffset_var = tk.StringVar(value='')
        self.stage_bound_var = tk.StringVar(value='320')
        left = ttk.Frame(stage, padding=(6, 0))
        left.grid(row=1, column=0, sticky='nsw')
        box = ttk.LabelFrame(left, text='Stage settings', padding=(6, 6))
        box.grid(row=0, column=0, sticky='ew')
        rows = [('image', self.stage_image_var), ('stage name', self.stage_name_var), ('output folder', self.stage_out_dir_var), ('zoffset blank=auto', self.stage_zoffset_var), ('bound width', self.stage_bound_var)]
        ...
        ttk.Button(box, text='Choose Background Image', command=self.stage_choose_image_ui).grid(row=len(rows), column=0, columnspan=2, sticky='ew', pady=2)
        ttk.Button(box, text='Choose Output Folder', command=self.stage_choose_output_ui).grid(row=len(rows)+1, column=0, columnspan=2, sticky='ew', pady=2)
        ttk.Button(box, text='Create Stage DEF + SFF', command=self.stage_create_ui).grid(row=len(rows)+2, column=0, columnspan=2, sticky='ew', pady=(8,2))

        right = ttk.Frame(stage)
        right.grid(row=1, column=1, sticky='nsew', padx=(0, 6), pady=(0, 6))
        right.rowconfigure(0, weight=1)
        right.columnconfigure(0, weight=1)
        self.stage_output = tk.Text(right, wrap='word', font=('Consolas', 10), state='disabled')
        y = ttk.Scrollbar(right, orient='vertical', command=self.stage_output.yview)
        self.stage_output.configure(yscrollcommand=y.set)
        self.stage_output.grid(row=0, column=0, sticky='nsew')
        y.grid(row=0, column=1, sticky='ns')
        self.notebook.add(stage, text='Stage Builder')
        self.set_text(self.stage_output, 'Pick one background image and an output folder. The generated stage is a starter, not a finished camera/parallax setup.')
...
'''
    ...
    # Forge Beyond v4.0 clean-room parity / beyond tools
    ...
    def _build_forge_beyond_tab(self):
        ...
        self.forge_beyond_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.forge_beyond_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=6)
        self.notebook.add(frame, text='Forge Beyond')
        self.set_text(self.forge_beyond_output, 'Forge Beyond is ready. Open a project, then run One-Click Parity+ Pass to generate the new reports, sheets, graphs, templates, and bundle.')
    ...
    def _forge_beyond_show_result(self, result, status: str):
        self.set_text(self.forge_beyond_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.notebook.select(self.forge_beyond_frame)
    ...
    def forge_beyond_one_click_ui(self):
        ...
        if not root:
            return
        try:
            result = run_forge_beyond_pass(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond parity+ pass complete')
            ...
            messagebox.showerror('Forge Beyond failed', str(exc))
    ...
    def forge_beyond_capability_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_write_capability_matrix(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond capability matrix written')
        except Exception as exc:
            messagebox.showerror('Capability Matrix failed', str(exc))
    ...
    def forge_beyond_index_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_write_unified_project_index(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond unified project index written')
        except Exception as exc:
            messagebox.showerror('Project Index failed', str(exc))
    ...
    def forge_beyond_state_graph_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_write_state_graph(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond state graph written')
        except Exception as exc:
            messagebox.showerror('State Graph failed', str(exc))
    ...
    def forge_beyond_variable_map_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_write_variable_usage_map(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond variable usage map written')
        except Exception as exc:
            messagebox.showerror('Variable Map failed', str(exc))
    ...
    def forge_beyond_cross_refs_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_write_sprite_sound_cross_reference(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond sprite/sound cross reference written')
        except Exception as exc:
            messagebox.showerror('Sprite/Sound Cross Reference failed', str(exc))
    ...
    def forge_beyond_export_command_sheet_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_export_command_sheet(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond command sheet exported')
        except Exception as exc:
            messagebox.showerror('Command Sheet export failed', str(exc))
    ...
    def forge_beyond_export_air_sheet_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_export_air_batch_sheet(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond AIR batch sheet exported')
        except Exception as exc:
            messagebox.showerror('AIR Batch Sheet export failed', str(exc))
    ...
    def forge_beyond_templates_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_write_template_plugin_foundation(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond template/plugin foundation written')
        except Exception as exc:
            messagebox.showerror('Template foundation failed', str(exc))
    ...
    def forge_beyond_stage_pack_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_write_stage_authoring_pack(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond stage authoring pack written')
        except Exception as exc:
            messagebox.showerror('Stage authoring pack failed', str(exc))
    ...
    def forge_beyond_release_bundle_ui(self):
        ...
        if not root:
            return
        try:
            result = fb_build_release_bundle(root)
            self.reload_project()
            self._forge_beyond_show_result(result, 'Forge Beyond reports ZIP built')
        except Exception as exc:
            messagebox.showerror('Reports ZIP failed', str(exc))

    # Forge Polish v4.1 workflow consolidation / polish tools
    def _build_forge_polish_tab(self):
        frame = ttk.Frame(self.notebook)
        self.forge_polish_frame = frame
        frame.columnconfigure(0, weight=1)
        ...
        self.set_text(self.forge_polish_output, 'Forge Polish v4.1 is ready. Open a project, then run One-Click Polish Pass or validate sheets before applying CSV changes.')
    ...
    def _forge_polish_show_result(self, result, status: str):
        self.set_text(self.forge_polish_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        try:
            self.notebook.select(self.forge_polish_frame)
        except Exception:
            pass
        self.status_var.set(status)
    ...
    # Forge Timeline v4.5 unified visual/sheet-backed editing workflows
    def _build_forge_timeline_tab(self):
        frame = ttk.Frame(self.notebook)
        self.forge_timeline_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)
        frame.rowconfigure(3, weight=1)

        header = ttk.LabelFrame(frame, text=f'Forge Timeline v{FORGE_TIMELINE_VERSION} — unified move, graph, palette, and stage cockpit', padding=(8, 6))
        header.grid(row=0, column=0, sticky='ew', padx=6, pady=6)
        header.columnconfigure(1, weight=1)
        ttk.Label(header, text=(
            'Source/sheet-backed visual workflows for move timing, CLSN boxes, HitDefs, state graph retargeting, palette variants, and approximate stage preview. '
            'Binary SFF/SND mutation remains conservative; generated reports are not engine-authoritative runtime telemetry.'
        ), wraplength=1040, justify='left').grid(row=0, column=0, columnspan=6, sticky='ew', pady=(0, 6))
        ttk.Label(header, text='Action filter').grid(row=1, column=0, sticky='e', padx=(0, 4))
        self.forge_timeline_action_var = tk.StringVar(value='')
        ttk.Entry(header, textvariable=self.forge_timeline_action_var, width=12).grid(row=1, column=1, sticky='w')
        ttk.Label(header, text='Leave blank for all actions. Frame rows use 0-based frame_index and M.U.G.E.N-style 1-based AnimElem.').grid(row=1, column=2, columnspan=4, sticky='w', padx=8)

        buttons = ttk.Frame(frame, padding=(6, 0))
        buttons.grid(row=1, column=0, sticky='ew')
        button_defs = [
            ('One-Click Timeline Pass', self.forge_timeline_one_click_ui),
            ('Dashboard', self.forge_timeline_dashboard_ui),
            ('Build Integrated Timeline', self.forge_timeline_build_model_ui),
            ('Export Move Sheet', self.forge_timeline_export_move_sheet_ui),
            ('Apply Move Sheet', self.forge_timeline_apply_move_sheet_ui),
            ('Export CLSN Sheet', self.forge_timeline_export_clsn_sheet_ui),
            ('Apply CLSN Sheet', self.forge_timeline_apply_clsn_sheet_ui),
            ('Export HitDef Sheet', self.forge_timeline_export_hitdef_sheet_ui),
            ('Apply HitDef Sheet', self.forge_timeline_apply_hitdef_sheet_ui),
            ('Build Editable State Graph', self.forge_timeline_state_graph_ui),
            ('Apply State Graph Sheet', self.forge_timeline_apply_state_graph_ui),
            ('Palette Studio', self.forge_timeline_palette_ui),
            ('Apply Palette Sheet', self.forge_timeline_apply_palette_ui),
            ('Stage Live Preview', self.forge_timeline_stage_preview_ui),
            ('Bundle Reports', self.forge_timeline_bundle_ui),
        ]
        for i, (label, cmd) in enumerate(button_defs):
            ttk.Button(buttons, text=label, command=cmd).grid(row=i // 5, column=i % 5, sticky='ew', padx=3, pady=3)
            buttons.columnconfigure(i % 5, weight=1)
        ...
        self.forge_timeline_canvas = tk.Canvas(preview_frame, bg='white', height=420, scrollregion=(0, 0, 1800, 1200))
        self.forge_timeline_canvas.grid(row=0, column=0, sticky='nsew')
        py = ttk.Scrollbar(preview_frame, orient='vertical', command=self.forge_timeline_canvas.yview)
        px = ttk.Scrollbar(preview_frame, orient='horizontal', command=self.forge_timeline_canvas.xview)
        py.grid(row=0, column=1, sticky='ns')
        px.grid(row=1, column=0, sticky='ew')
        self.forge_timeline_canvas.configure(yscrollcommand=py.set, xscrollcommand=px.set)
        body.add(preview_frame, weight=3)

        help_frame = ttk.Frame(body)
        help_frame.columnconfigure(0, weight=1)
        help_frame.rowconfigure(0, weight=1)
        self.forge_timeline_help = tk.Text(help_frame, wrap='word', font=('Consolas', 10), state='disabled', width=44)
        self.forge_timeline_help.grid(row=0, column=0, sticky='nsew')
        body.add(help_frame, weight=1)
        self.set_text(self.forge_timeline_help, (
            'Forge Timeline v4.5 workflow:\n\n'
            '1. Run One-Click Timeline Pass.\n'
            '2. Open forge_timeline/FORGE_TIMELINE_START_HERE.md.\n'
            '3. Edit CSV sheets and set only intended rows to apply=yes.\n'
            '4. Apply sheets from this tab.\n'
            '5. Review backups and playtest in M.U.G.E.N.\n\n'
            'This is source/sheet-backed editing. It does not implement full arbitrary SFF v2 binary editing/rebuilding.'
        ))

        self.forge_timeline_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled', height=11)
        self.forge_timeline_output.grid(row=3, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.forge_timeline_preview_photo = None
        self.notebook.add(frame, text='Forge Timeline')
        self.set_text(self.forge_timeline_output, 'Forge Timeline v4.5 is ready. Open a project, then run One-Click Timeline Pass.')

    def _forge_timeline_root(self):
        if not self.project_root:
            messagebox.showinfo('Forge Timeline', 'Open or create a project folder first.')
            return None
        return self.project_root

    def _forge_timeline_action_filter(self):
        raw = getattr(self, 'forge_timeline_action_var', tk.StringVar(value='')).get().strip()
        if not raw:
            return None
        try:
            return int(raw)
        except Exception:
            messagebox.showwarning('Action filter', 'Action filter must be blank or an integer action number.')
            return None

    def _forge_timeline_show_result(self, result, status: str):
        self.set_text(self.forge_timeline_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.forge_timeline_frame)
        except Exception:
            pass

    def _forge_timeline_load_preview_image(self, path: Path):
        canvas = self.forge_timeline_canvas
        canvas.delete('all')
        path = Path(path)
        if not path.exists():
            canvas.create_text(20, 20, anchor='nw', text=f'Preview not found:\n{path}', fill='black')
        ...
        try:
            ...
            self.forge_timeline_preview_photo = ImageTk.PhotoImage(img)
            canvas.create_image(10, 10, anchor='nw', image=self.forge_timeline_preview_photo)
            canvas.configure(scrollregion=(0, 0, max(900, img.width + 30), max(600, img.height + 30)))
        except Exception:
            try:
                self.forge_timeline_preview_photo = tk.PhotoImage(file=str(path))
                canvas.create_image(10, 10, anchor='nw', image=self.forge_timeline_preview_photo)
            except Exception as exc:
                canvas.create_text(20, 20, anchor='nw', text=f'Could not load preview:\n{path}\n{exc}', fill='black')

    def forge_timeline_one_click_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = run_forge_timeline_pass(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline v4.5 pass complete')
            self._forge_timeline_load_preview_image(root / 'forge_timeline' / 'move_timeline' / 'integrated_move_timeline.png')
            messagebox.showinfo('Forge Timeline complete', 'v4.5 timeline pass completed. Open forge_timeline/FORGE_TIMELINE_START_HERE.md to start.')
        except Exception as exc:
            messagebox.showerror('Forge Timeline failed', str(exc))

    def forge_timeline_dashboard_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = write_forge_timeline_dashboard(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline dashboard written')
        except Exception as exc:
            messagebox.showerror('Dashboard failed', str(exc))

    def forge_timeline_build_model_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = build_integrated_move_timeline_model(root, self._forge_timeline_action_filter())
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline integrated model built')
            self._forge_timeline_load_preview_image(root / 'forge_timeline' / 'move_timeline' / 'integrated_move_timeline.png')
        except Exception as exc:
            messagebox.showerror('Timeline model failed', str(exc))

    def forge_timeline_export_move_sheet_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = export_move_timeline_edit_sheet(root, self._forge_timeline_action_filter())
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline move sheet exported')
        except Exception as exc:
            messagebox.showerror('Move sheet export failed', str(exc))

    def forge_timeline_apply_move_sheet_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = apply_move_timeline_edit_sheet(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline move sheet applied')
        except Exception as exc:
            messagebox.showerror('Move sheet apply failed', str(exc))

    def forge_timeline_export_clsn_sheet_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = export_clsn_track_sheet(root, self._forge_timeline_action_filter())
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline CLSN sheet exported')
        except Exception as exc:
            messagebox.showerror('CLSN sheet export failed', str(exc))

    def forge_timeline_apply_clsn_sheet_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = apply_clsn_track_sheet(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline CLSN sheet applied')
        except Exception as exc:
            messagebox.showerror('CLSN sheet apply failed', str(exc))

    def forge_timeline_export_hitdef_sheet_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = export_hitdef_track_sheet(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline HitDef sheet exported')
        except Exception as exc:
            messagebox.showerror('HitDef sheet export failed', str(exc))

    def forge_timeline_apply_hitdef_sheet_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = apply_hitdef_track_sheet(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline HitDef sheet applied')
        except Exception as exc:
            messagebox.showerror('HitDef sheet apply failed', str(exc))

    def forge_timeline_state_graph_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = build_editable_state_graph(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline editable state graph built')
            self._forge_timeline_load_preview_image(root / 'forge_timeline' / 'state_graph_editor' / 'editable_state_graph.png')
        except Exception as exc:
            messagebox.showerror('State graph failed', str(exc))

    def forge_timeline_apply_state_graph_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = apply_state_graph_edit_sheet(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline state graph sheet applied')
        except Exception as exc:
            messagebox.showerror('State graph apply failed', str(exc))

    def forge_timeline_palette_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        palette = None
        if messagebox.askyesno('Palette Studio', 'Choose a specific ACT palette? Choose No to auto-detect or create a starter palette.'):
            chosen = filedialog.askopenfilename(title='Choose ACT palette', filetypes=[('ACT palettes', '*.act'), ('All files', '*.*')])
            if chosen:
                palette = Path(chosen)
        try:
            result = write_palette_studio(root, palette_path=palette)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline palette studio written')
            self._forge_timeline_load_preview_image(root / 'forge_timeline' / 'palette_studio' / 'palette_grid.png')
        except Exception as exc:
            messagebox.showerror('Palette Studio failed', str(exc))

    def forge_timeline_apply_palette_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = apply_palette_editor_sheet(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline palette sheet applied')
        except Exception as exc:
            messagebox.showerror('Palette apply failed', str(exc))

    def forge_timeline_stage_preview_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        image = None
        if messagebox.askyesno('Stage Live Preview', 'Choose a specific source/background image? Choose No to auto-detect.'):
            chosen = filedialog.askopenfilename(title='Choose stage/source image', filetypes=[('Images', '*.png *.pcx *.bmp *.gif *.jpg *.jpeg *.webp'), ('All files', '*.*')])
            if chosen:
                image = Path(chosen)
        try:
            result = write_stage_live_preview(root, image_path=image)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline stage live preview written')
            self._forge_timeline_load_preview_image(root / 'forge_timeline' / 'stage_live_preview' / 'stage_live_preview.png')
        except Exception as exc:
            messagebox.showerror('Stage preview failed', str(exc))

    def forge_timeline_bundle_ui(self):
        root = self._forge_timeline_root()
        if not root:
            return
        try:
            result = build_forge_timeline_bundle(root)
            self.reload_project()
            self._forge_timeline_show_result(result, 'Forge Timeline reports bundle built')
        except Exception as exc:
            messagebox.showerror('Timeline bundle failed', str(exc))


    # Binary Core v5.5 native binary inspection, safe SFF/SND workflows
    def _build_binary_core_tab(self):
        frame = ttk.Frame(self.notebook)
        self.binary_core_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)

        header = ttk.LabelFrame(frame, text=f'Binary Core v{BINARY_CORE_VERSION} — SFF/SND inspection, safe rebuilds, backups, and roundtrip reports', padding=(8, 6))
        header.grid(row=0, column=0, sticky='ew', padx=6, pady=6)
        header.columnconfigure(1, weight=1)
        intro = (
        ...
        ttk.Button(header, text='One-Click Binary Core Pass', command=self.binary_core_one_click_ui).grid(row=1, column=0, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Dashboard', command=self.binary_core_dashboard_ui).grid(row=1, column=1, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Safety Report', command=self.binary_core_safety_ui).grid(row=1, column=2, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Roundtrip Report', command=self.binary_core_roundtrip_ui).grid(row=1, column=3, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Bundle Reports', command=self.binary_core_bundle_ui).grid(row=1, column=4, padx=3, pady=2, sticky='ew')

        body = ttk.Panedwindow(frame, orient=tk.HORIZONTAL)
        body.grid(row=1, column=0, sticky='nsew', padx=6, pady=(0, 6))

        left = ttk.LabelFrame(body, text='SFF / SFF2 Binary Tools', padding=(6, 6))
        left.columnconfigure(0, weight=1)
        for i, (label, cmd) in enumerate([
            ...
            ttk.Button(left, text=label, command=cmd).grid(row=i, column=0, sticky='ew', padx=2, pady=2)
        body.add(left, weight=1)

        right = ttk.LabelFrame(body, text='SND Bank Tools', padding=(6, 6))
        right.columnconfigure(0, weight=1)
        for i, (label, cmd) in enumerate([
            ...
            ttk.Button(right, text=label, command=cmd).grid(row=i, column=0, sticky='ew', padx=2, pady=2)
        body.add(right, weight=1)

        self.binary_core_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.binary_core_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.notebook.add(frame, text='Binary Core')
        self.set_text(self.binary_core_output, 'Binary Core v5.5 is ready. Open a project, then run One-Click Binary Core Pass or inspect SFF/SND assets individually.')

    def _binary_core_root(self):
        if not self.project_root:
            messagebox.showinfo('Binary Core', 'Open or create a project folder first.')
            return None
        return self.project_root

    def _binary_core_show_result(self, result, status: str):
        self.set_text(self.binary_core_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.binary_core_frame)
        except Exception:
            pass
    ...
    def binary_core_one_click_ui(self):
        ...
        if not root:
            return
        try:
            result = run_binary_core_pass(root)
            self.reload_project()
    ...
    def binary_core_dashboard_ui(self):
        ...
        if not root:
            return
        try:
            result = write_binary_core_dashboard(root)
            self.reload_project()
            self._binary_core_show_result(result, 'Binary Core dashboard written')
        except Exception as exc:
            messagebox.showerror('Binary Core dashboard failed', str(exc))
    ...
    def binary_core_safety_ui(self):
        ...
        if not root:
            return
        try:
            result = write_binary_safety_report(root)
            self.reload_project()
            self._binary_core_show_result(result, 'Binary safety report written')
        except Exception as exc:
            messagebox.showerror('Binary safety report failed', str(exc))
    ...
    def binary_core_sff2_inspect_ui(self):
        ...
        if not root:
            return
        try:
            result = inspect_sff2_native(root)
            self.reload_project()
            self._binary_core_show_result(result, 'SFF2 native inspection complete')
        except Exception as exc:
            messagebox.showerror('SFF2 inspection failed', str(exc))
    ...
    def binary_core_sff2_export_ui(self):
        ...
        if not root:
            return
        try:
            result = export_supported_sff2_payloads(root)
            self.reload_project()
            self._binary_core_show_result(result, 'Supported SFF payload export complete')
        except Exception as exc:
            messagebox.showerror('SFF payload export failed', str(exc))
    ...
    def binary_core_export_axis_ui(self):
        ...
        if not root:
            return
        try:
            result = export_sff_axis_sheet(root)
            self.reload_project()
            self._binary_core_show_result(result, 'SFF axis sheet exported')
        except Exception as exc:
            messagebox.showerror('SFF axis sheet export failed', str(exc))

    def binary_core_apply_axis_ui(self):
        root = self._binary_core_root()
        if not root:
            return
        if not messagebox.askyesno('Apply SFF Axis Sheet', 'Apply enabled SFF axis rows from binary_core/sff_axis_editor/sff_axis_edit_sheet.csv? This creates a patched copy; the original SFF is not overwritten.'):
            return
        try:
            result = apply_sff_axis_sheet(root)
            self.reload_project()
            self._binary_core_show_result(result, 'SFF axis sheet applied')
        except Exception as exc:
            messagebox.showerror('SFF axis apply failed', str(exc))
    ...
    def binary_core_sff2_rebuild_pack_ui(self):
        ...
        if messagebox.askyesno('SFF2 Source Rebuild Pack', 'Choose a source sprite image folder? Choose No to auto-detect source_sprites/sprites/staged_sprites.'):
            ...
            if chosen:
                folder = Path(chosen)
        try:
            result = create_sff2_source_rebuild_pack(root, folder)
            self.reload_project()
    ...
    def binary_core_sff2_png_copy_ui(self):
        ...
        if not root:
            return
        if not messagebox.askyesno('Experimental SFF2 PNG Copy', 'Build the experimental MugenForge PNG-subset SFF2-style copy from the source workspace? This is for pipeline testing, not a full Sprmake2 replacement.'):
            return
        try:
            result = build_sff2_png_workspace_copy(root)
            self.reload_project()
            self._binary_core_show_result(result, 'Experimental SFF2 PNG copy built')
        except Exception as exc:
            messagebox.showerror('Experimental SFF2 PNG copy failed', str(exc))
    ...
    def binary_core_export_snd_ui(self):
        ...
        if not root:
            return
        try:
            result = export_snd_bank_sheet(root)
            self.reload_project()
            self._binary_core_show_result(result, 'SND bank sheet exported')
        except Exception as exc:
            messagebox.showerror('SND bank sheet export failed', str(exc))

    def binary_core_apply_snd_ui(self):
        root = self._binary_core_root()
        if not root:
            return
        if not messagebox.askyesno('Apply SND Bank Sheet', 'Create a rebuilt SND candidate from binary_core/snd_editor/snd_bank_edit_sheet.csv? The current SND is not overwritten.'):
            return
        try:
            result = apply_snd_bank_sheet(root)
            self.reload_project()
            self._binary_core_show_result(result, 'SND bank sheet applied')
        except Exception as exc:
            messagebox.showerror('SND bank apply failed', str(exc))
    ...
    def binary_core_snd_waveform_ui(self):
        ...
        if not root:
            return
        try:
            result = write_snd_waveform_preview(root)
            self.reload_project()
            self._binary_core_show_result(result, 'SND waveform preview written')
        except Exception as exc:
            messagebox.showerror('SND waveform preview failed', str(exc))
    ...
    def binary_core_roundtrip_ui(self):
        ...
        if not root:
            return
        try:
            result = build_binary_roundtrip_report(root)
            self.reload_project()
            self._binary_core_show_result(result, 'Binary roundtrip report written')
        except Exception as exc:
            messagebox.showerror('Binary roundtrip report failed', str(exc))
    ...
    def binary_core_bundle_ui(self):
        ...
        if not root:
            return
        try:
            result = build_binary_core_bundle(root)
            self.reload_project()
            self._binary_core_show_result(result, 'Binary Core reports bundle built')
        except Exception as exc:
            messagebox.showerror('Binary Core bundle failed', str(exc))


    # Binary Deep v5.5 expanded SFF2/SND mutation, decode, runtime validation workflows
    def _build_binary_deep_tab(self):
        frame = ttk.Frame(self.notebook)
        self.binary_deep_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)

        header = ttk.LabelFrame(frame, text=f'Binary Deep v{BINARY_DEEP_VERSION} — expanded SFF2 decoding/mutation, SND patch candidates, runtime validation', padding=(8, 6))
        header.grid(row=0, column=0, sticky='ew', padx=6, pady=6)
        header.columnconfigure(1, weight=1)
        intro = (
        ...
        ttk.Button(header, text='One-Click Binary Deep Pass', command=self.binary_deep_one_click_ui).grid(row=1, column=0, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Dashboard', command=self.binary_deep_dashboard_ui).grid(row=1, column=1, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Runtime Validation Lab', command=self.binary_deep_runtime_ui).grid(row=1, column=2, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Bundle Reports', command=self.binary_deep_bundle_ui).grid(row=1, column=3, padx=3, pady=2, sticky='ew')

        body = ttk.Panedwindow(frame, orient=tk.HORIZONTAL)
        body.grid(row=1, column=0, sticky='nsew', padx=6, pady=(0, 6))

        left = ttk.LabelFrame(body, text='Deep SFF2 Tools', padding=(6, 6))
        left.columnconfigure(0, weight=1)
        for i, (label, cmd) in enumerate([
        ...
        right = ttk.LabelFrame(body, text='Deep SND Tools', padding=(6, 6))
        right.columnconfigure(0, weight=1)
        for i, (label, cmd) in enumerate([
        ...
        self.binary_deep_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.binary_deep_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.notebook.add(frame, text='Binary Deep')
        self.set_text(self.binary_deep_output, 'Binary Deep v5.5 is ready. Open a project, then run One-Click Binary Deep Pass or inspect SFF/SND assets individually.')

    def _binary_deep_root(self):
        if not self.project_root:
            messagebox.showinfo('Binary Deep', 'Open or create a project folder first.')
            return None
        return self.project_root

    def _binary_deep_show_result(self, result, status: str):
        self.set_text(self.binary_deep_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.binary_deep_frame)
        except Exception:
            pass
    ...
    def binary_deep_one_click_ui(self):
        ...
        if not root:
            return
        try:
            result = run_binary_deep_pass(root)
            self.reload_project()
    ...
    def binary_deep_dashboard_ui(self):
        ...
        if not root:
            return
        try:
            result = write_binary_deep_dashboard(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'Binary Deep dashboard written')
        except Exception as exc:
            messagebox.showerror('Binary Deep dashboard failed', str(exc))
    ...
    def binary_deep_sff2_inspect_ui(self):
        ...
        if not root:
            return
        try:
            result = inspect_sff2_deep(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'Deep SFF2 inspection complete')
        except Exception as exc:
            messagebox.showerror('Deep SFF2 inspection failed', str(exc))
    ...
    def binary_deep_sff2_export_ui(self):
        ...
        if not root:
            return
        try:
            result = export_sff2_deep_sprites(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'Deep SFF2 export complete')
        except Exception as exc:
            messagebox.showerror('Deep SFF2 export failed', str(exc))
    ...
    def binary_deep_export_mutation_ui(self):
        ...
        if not root:
            return
        try:
            result = export_sff2_mutation_sheet(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'SFF2 mutation sheet exported')
        except Exception as exc:
            messagebox.showerror('SFF2 mutation sheet export failed', str(exc))

    def binary_deep_apply_mutation_ui(self):
        root = self._binary_deep_root()
        if not root:
            return
        if not messagebox.askyesno('Apply SFF2 Mutation Sheet', 'Apply enabled rows from binary_deep/sff2_mutation/deep_sff2_mutation_sheet.csv? This creates candidate patched/rebuilt files first.'):
            return
        try:
            result = apply_sff2_mutation_sheet(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'SFF2 mutation sheet applied')
        except Exception as exc:
            messagebox.showerror('SFF2 mutation sheet apply failed', str(exc))
    ...
    def binary_deep_build_native_sff2_ui(self):
        ...
        if not root:
            return
        try:
            result = build_native_sff2(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'Native SFF2 build completed')
        except Exception as exc:
            messagebox.showerror('Native SFF2 build failed', str(exc))
    ...
    def binary_deep_export_snd_patch_ui(self):
        ...
        if not root:
            return
        try:
            result = export_snd_patch_sheet(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'SND direct patch sheet exported')
        except Exception as exc:
            messagebox.showerror('SND direct patch sheet export failed', str(exc))

    def binary_deep_apply_snd_patch_ui(self):
        root = self._binary_deep_root()
        if not root:
            return
        if not messagebox.askyesno('Apply SND Direct Patch Sheet', 'Apply enabled rows from binary_deep/snd_direct_patch/snd_direct_patch_sheet.csv? This creates a patched candidate copy first.'):
            return
        try:
            result = apply_snd_patch_sheet(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'SND direct patch sheet applied')
        except Exception as exc:
            messagebox.showerror('SND direct patch apply failed', str(exc))
    ...
    def binary_deep_runtime_ui(self):
        ...
        if not root:
            return
        try:
            result = write_runtime_lab(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'Runtime validation lab written')
        except Exception as exc:
            messagebox.showerror('Runtime validation lab failed', str(exc))
    ...
    def binary_deep_bundle_ui(self):
        ...
        if not root:
            return
        try:
            result = build_binary_deep_bundle(root)
            self.reload_project()
            self._binary_deep_show_result(result, 'Binary Deep reports bundle built')
        except Exception as exc:
            messagebox.showerror('Binary Deep bundle failed', str(exc))


    # Binary Maturity v5.5: standard SFF2 parser/rebuilder, honest decode boundaries, SND slot patch copies, runtime harness
    def _build_binary_maturity_tab(self):
        frame = ttk.Frame(self.notebook)
        self.binary_maturity_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)

        header = ttk.LabelFrame(frame, text=f'Binary Maturity v{BINARY_MATURITY_VERSION} — standard SFF2, safe rebuild candidates, SND slot patches, runtime harness', padding=(8, 6))
        header.grid(row=0, column=0, sticky='ew', padx=6, pady=6)
        header.columnconfigure(1, weight=1)
        intro = (
        ...
        ttk.Label(header, text=intro, wraplength=1180, justify='left').grid(row=0, column=0, columnspan=5, sticky='ew', pady=(0, 6))
        ttk.Button(header, text='One-Click Binary Maturity Pass', command=self.binary_maturity_one_click_ui).grid(row=1, column=0, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='SFF2 Maturity Report', command=self.binary_maturity_sff2_report_ui).grid(row=1, column=1, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Runtime Harness', command=self.binary_maturity_runtime_harness_ui).grid(row=1, column=2, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Run Runtime Harness', command=self.binary_maturity_run_runtime_ui).grid(row=1, column=3, padx=3, pady=2, sticky='ew')
        ttk.Button(header, text='Bundle Reports', command=self.binary_maturity_bundle_ui).grid(row=1, column=4, padx=3, pady=2, sticky='ew')

        body = ttk.Panedwindow(frame, orient=tk.HORIZONTAL)
        body.grid(row=1, column=0, sticky='nsew', padx=6, pady=(0, 6))

        left = ttk.LabelFrame(body, text='SFF2 Maturity Tools', padding=(6, 6))
        left.columnconfigure(0, weight=1)
        for i, (label, cmd) in enumerate([
        ...
        right = ttk.LabelFrame(body, text='SND Maturity Tools', padding=(6, 6))
        right.columnconfigure(0, weight=1)
        for i, (label, cmd) in enumerate([
        ...
        self.binary_maturity_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.binary_maturity_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.notebook.add(frame, text='Binary Maturity')
        self.set_text(self.binary_maturity_output, 'Binary Maturity v5.5 is ready. Open a project, then run One-Click Binary Maturity Pass or use the focused SFF2/SND/runtime tools.')

    def _binary_maturity_root(self):
        if not self.project_root:
            messagebox.showinfo('Binary Maturity', 'Open or create a project folder first.')
            return None
        return self.project_root

    def _binary_maturity_show_result(self, result, status: str):
        self.set_text(self.binary_maturity_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.binary_maturity_frame)
        except Exception:
            pass
    ...
    def binary_maturity_one_click_ui(self):
        ...
        if not root:
            return
        try:
            result = run_binary_maturity_pass(root)
            self.reload_project()
    ...
    def binary_maturity_sff2_report_ui(self):
        ...
        if not root:
            return
        try:
            result = write_sff2_maturity_report(root)
            self.reload_project()
            self._binary_maturity_show_result(result, 'SFF2 maturity report written')
        except Exception as exc:
            messagebox.showerror('SFF2 maturity report failed', str(exc))
    ...
    def binary_maturity_export_sff2_workspace_ui(self):
        ...
        if not root:
            return
        try:
            result = export_sff2_edit_workspace(root)
            self.reload_project()
            self._binary_maturity_show_result(result, 'SFF2 edit workspace exported')
        except Exception as exc:
            messagebox.showerror('SFF2 edit workspace export failed', str(exc))

    def binary_maturity_apply_sff2_workspace_ui(self):
        root = self._binary_maturity_root()
        if not root:
            return
        if not messagebox.askyesno('Apply SFF2 Edit Workspace', 'Build a new SFF2 candidate from binary_maturity/sff2_edit_workspace/sff2_sprite_edit_sheet.csv? The original SFF is not overwritten.'):
            return
        try:
            result = apply_sff2_edit_workspace(root)
            self.reload_project()
            self._binary_maturity_show_result(result, 'SFF2 edit workspace applied')
        except Exception as exc:
            messagebox.showerror('SFF2 edit workspace apply failed', str(exc))
    ...
    def binary_maturity_build_sff2_source_ui(self):
        ...
        try:
            result = build_sff2_from_source_folder(root, Path(folder) if folder else None)
            self.reload_project()
    ...
    def binary_maturity_export_snd_slot_ui(self):
        ...
        if not root:
            return
        try:
            result = export_snd_slot_patch_sheet(root)
            self.reload_project()
            self._binary_maturity_show_result(result, 'SND slot patch sheet exported')
        except Exception as exc:
            messagebox.showerror('SND slot sheet export failed', str(exc))

    def binary_maturity_apply_snd_slot_ui(self):
        root = self._binary_maturity_root()
        if not root:
            return
        if not messagebox.askyesno('Apply SND Slot Patch Sheet', 'Apply enabled same-or-smaller WAV slot patches into a candidate SND copy? The original SND is not overwritten.'):
            return
        try:
            result = apply_snd_slot_patch_sheet(root)
            self.reload_project()
            self._binary_maturity_show_result(result, 'SND slot patch sheet applied')
        except Exception as exc:
            messagebox.showerror('SND slot patch apply failed', str(exc))
    ...
    def binary_maturity_runtime_harness_ui(self):
        ...
        if not root:
            return
        try:
            result = write_runtime_test_harness(root)
            self.reload_project()
            self._binary_maturity_show_result(result, 'Runtime harness written')
        except Exception as exc:
            messagebox.showerror('Runtime harness write failed', str(exc))
    ...
    def binary_maturity_run_runtime_ui(self):
        ...
        if not root:
            return
        try:
            result = run_runtime_test_harness(root)
            self.reload_project()
            self._binary_maturity_show_result(result, 'Runtime harness run complete')
        except Exception as exc:
            messagebox.showerror('Runtime harness run failed', str(exc))
    ...
    def binary_maturity_bundle_ui(self):
        ...
        if not root:
            return
        try:
            result = build_binary_maturity_bundle(root)
            self.reload_project()
            self._binary_maturity_show_result(result, 'Binary Maturity reports bundle built')
        except Exception as exc:
            messagebox.showerror('Binary Maturity bundle failed', str(exc))
    ...
    def _build_gap_closer_tab(self):
        frame = ttk.Frame(self.notebook)
        self.gap_closer_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)
        header = ttk.LabelFrame(frame, text=f'Gap Closer v{GAP_CLOSER_VERSION} — closes previous honest limitations with concrete tools', padding=(8, 6))
        header.grid(row=0, column=0, sticky='ew', padx=6, pady=6)
        desc = (
        ...
        self.gap_closer_note = tk.Text(frame, wrap='word', height=5, font=('Consolas', 9), state='disabled')
        self.gap_closer_note.grid(row=1, column=0, sticky='ew', padx=6, pady=(0, 6))
        self.set_text(self.gap_closer_note, 'Start with One-Click Gap Closer Pass. For live engine evidence, configure/probe an engine executable first. Guarded binary commits require enabled=yes and commit_token=COMMIT in the CSV sheet.')
        self.gap_closer_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.gap_closer_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.notebook.add(frame, text='Gap Closer')
        self.set_text(self.gap_closer_output, f'Gap Closer v{GAP_CLOSER_VERSION} is ready. Open a project, then run One-Click Gap Closer Pass.')

    def _gap_closer_root(self):
        root = self.project_root or (self.current_path.parent if self.current_path else None)
        if not root:
    ...
    def _gap_closer_show_result(self, result, status: str):
        self.set_text(self.gap_closer_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.gap_closer_frame)
        except Exception:
            pass
    ...
    def gap_closer_one_click_ui(self):
        ...
        if not root:
            return
        try:
            result = run_gap_closer_pass(root)
            self.reload_project()
    ...
    def gap_closer_dashboard_ui(self):
        ...
        if not root:
            return
        try:
            result = write_gap_closer_dashboard(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Gap Closer dashboard written')
        except Exception as exc:
            messagebox.showerror('Gap Closer dashboard failed', str(exc))
    ...
    def gap_closer_engine_probe_ui(self):
        ...
        try:
            result = probe_engine_profile(root, engine or None)
            self.reload_project()
    ...
    def gap_closer_source_model_ui(self):
        ...
        if not root:
            return
        try:
            result = build_source_runtime_model(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Source runtime model written')
        except Exception as exc:
            messagebox.showerror('Source runtime model failed', str(exc))
    ...
    def gap_closer_scenario_dry_ui(self):
        ...
        if not root:
            return
        try:
            result = build_engine_scenario_sessions(root, dry_run=True)
            self.reload_project()
            self._gap_closer_show_result(result, 'Engine scenario dry-run written')
        except Exception as exc:
            messagebox.showerror('Engine scenario dry-run failed', str(exc))
    ...
    def gap_closer_scenario_live_ui(self):
        ...
        try:
            ...
            result = build_engine_scenario_sessions(root, dry_run=False, timeout_seconds=timeout)
            self.reload_project()
    ...
    def gap_closer_readiness_ui(self):
        ...
        if not root:
            return
        try:
            result = build_engine_backed_readiness_gate(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Engine-backed readiness gate written')
        except Exception as exc:
            messagebox.showerror('Engine-backed readiness failed', str(exc))
    ...
    def gap_closer_corpus_ui(self):
        ...
        try:
            result = build_sff_snd_corpus_lab(root, folder or None)
            self.reload_project()
    ...
    def gap_closer_triage_ui(self):
        ...
        if not root:
            return
        try:
            result = write_unknown_binary_triage(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Unknown binary triage written')
        except Exception as exc:
            messagebox.showerror('Unknown binary triage failed', str(exc))
    ...
    def gap_closer_commit_sheet_ui(self):
        ...
        if not root:
            return
        try:
            result = export_binary_commit_sheet(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Guarded binary commit sheet exported')
        except Exception as exc:
            messagebox.showerror('Guarded binary commit sheet failed', str(exc))

    def gap_closer_apply_commit_ui(self):
        root = self._gap_closer_root()
        if not root:
            return
        if not messagebox.askyesno('Apply guarded binary commits?', 'Only enabled rows with commit_token=COMMIT will be copied into target binary slots after hash checks and backups. Continue?'):
            return
        try:
            result = apply_binary_commit_sheet(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Guarded binary commit sheet applied')
        except Exception as exc:
            messagebox.showerror('Guarded binary commit apply failed', str(exc))
    ...
    def gap_closer_tuning_sheet_ui(self):
        ...
        if not root:
            return
        try:
            result = export_generated_code_tuning_lab(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Generated code tuning sheet exported')
        except Exception as exc:
            messagebox.showerror('Generated code tuning sheet failed', str(exc))

    def gap_closer_apply_tuning_ui(self):
        root = self._gap_closer_root()
        if not root:
            return
        if not messagebox.askyesno('Apply generated-code tuning?', 'Only enabled rows in generated_code_tuning_sheet.csv will be applied, with backups. Continue?'):
            return
        try:
            result = apply_generated_code_tuning_sheet(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Generated code tuning sheet applied')
        except Exception as exc:
            messagebox.showerror('Generated code tuning apply failed', str(exc))
    ...
    def gap_closer_bundle_ui(self):
        ...
        if not root:
            return
        try:
            result = build_gap_closer_bundle(root)
            self.reload_project()
            self._gap_closer_show_result(result, 'Gap Closer bundle built')
        except Exception as exc:
            messagebox.showerror('Gap Closer bundle failed', str(exc))


    # Authority Core v7.0: engine-backed evidence, corpus validation, verified installs, and tuning feedback.
    def _build_authority_core_tab(self):
        frame = ttk.Frame(self.notebook)
        self.authority_core_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)
        ...
        self.authority_core_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.authority_core_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.notebook.add(frame, text='Authority Core')
        self.set_text(self.authority_core_output, f'Authority Core v{ac.AUTHORITY_CORE_VERSION} is ready. Start with One-Click Authority Pass, then configure authority_core/engine_config.json for real engine-backed runs.')
    ...
    def _authority_core_show_result(self, result, status: str):
        self.set_text(self.authority_core_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.authority_core_frame)
        except Exception:
            pass
    ...
    def authority_core_contracts_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.write_engine_contract_profiles(root), 'Engine contract profiles written')
        except Exception as exc:
            messagebox.showerror('Engine contracts failed', str(exc))
    ...
    def authority_core_validate_engine_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.validate_engine_configuration(root), 'Engine config validated')
        except Exception as exc:
            messagebox.showerror('Engine config validation failed', str(exc))
    ...
    def authority_core_parse_engine_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.parse_engine_run_results(root), 'Engine run results parsed')
        except Exception as exc:
            messagebox.showerror('Engine result parsing failed', str(exc))
    ...
    def authority_core_reconcile_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.write_runtime_reconciliation_report(root), 'Runtime reconciliation written')
        except Exception as exc:
            messagebox.showerror('Runtime reconciliation failed', str(exc))
    ...
    def authority_core_triage_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.triage_unknown_binary_layouts(root), 'Unknown binary triage written')
        except Exception as exc:
            messagebox.showerror('Unknown binary triage failed', str(exc))
    ...
    def authority_core_install_sheet_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.export_binary_install_sheet(root), 'Binary install sheet written')
        except Exception as exc:
            messagebox.showerror('Binary install sheet failed', str(exc))
    ...
    def authority_core_tuning_sheet_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.write_gameplay_tuning_feedback_sheet(root), 'Gameplay tuning feedback sheet written')
        except Exception as exc:
            messagebox.showerror('Gameplay tuning sheet failed', str(exc))
    ...
    def authority_core_score_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.write_authority_score(root), 'Authority score written')
        except Exception as exc:
            messagebox.showerror('Authority score failed', str(exc))
    ...
    def authority_core_bundle_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_core_show_result(ac.build_authority_core_bundle(root), 'Authority Core bundle built')
        except Exception as exc:
            messagebox.showerror('Authority bundle failed', str(exc))

    # Authority Lab v7.0 — evidence-backed release gate, source-runtime simulation, corpus validation, and verified binary commit.
    def _build_authority_lab_tab(self):
        frame = ttk.Frame(self.notebook)
        self.authority_lab_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)
        ...
        self.authority_lab_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.authority_lab_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.notebook.add(frame, text='Authority Lab')
        self.set_text(self.authority_lab_output, 'Authority Lab v7.0 is ready. Open a project, then run One-Click Authority Pass.')
    ...
    def _authority_lab_show_result(self, result, status: str):
        self.set_text(self.authority_lab_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.authority_lab_frame)
        except Exception:
            pass
        try:
            self.reload_project()
    ...
    def authority_lab_dashboard_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_lab_show_result(auth_lab.write_authority_dashboard(root), 'Authority dashboard written')
        except Exception as exc:
            messagebox.showerror('Authority dashboard failed', str(exc))
    ...
    def authority_lab_verify_engine_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_lab_show_result(auth_lab.verify_engine_profile(root, run_probe=False), 'Engine profile verified')
        except Exception as exc:
            messagebox.showerror('Engine verification failed', str(exc))
    ...
    def authority_lab_source_sim_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_lab_show_result(auth_lab.build_source_runtime_simulation(root), 'Source runtime simulation written')
        except Exception as exc:
            messagebox.showerror('Source runtime simulation failed', str(exc))
    ...
    def authority_lab_commit_sheet_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_lab_show_result(auth_lab.export_verified_binary_commit_sheet(root), 'Verified binary commit sheet written')
        except Exception as exc:
            messagebox.showerror('Binary commit sheet failed', str(exc))
    ...
    def authority_lab_tuning_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_lab_show_result(auth_lab.write_gameplay_tuning_workbench(root), 'Gameplay tuning workbench written')
        except Exception as exc:
            messagebox.showerror('Gameplay tuning workbench failed', str(exc))
    ...
    def authority_lab_gate_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_lab_show_result(auth_lab.write_evidence_authority_gate(root), 'Evidence authority gate written')
        except Exception as exc:
            messagebox.showerror('Evidence authority gate failed', str(exc))
    ...
    def authority_lab_bundle_ui(self):
        ...
        if not root:
            return
        try:
            self._authority_lab_show_result(auth_lab.build_authority_bundle(root), 'Authority bundle built')
        except Exception as exc:
            messagebox.showerror('Authority bundle failed', str(exc))


    # Runtime Lab v6.1 — launch/test harness, logs, telemetry, and evidence scoring.

    # Closure Lab v7.0 — targeted closure tools for the remaining limitation list.
    def _build_closure_lab_tab(self):
        frame = ttk.Frame(self.notebook)
        self.closure_lab_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)

        header = ttk.LabelFrame(frame, text=f'Closure Lab v{CLOSURE_LAB_VERSION} — runtime simulation, engine adapters, corpus validation, safe commit, tuning loop', padding=(8, 6))
        header.grid(row=0, column=0, sticky='ew', padx=6, pady=6)
        intro = (
        ...
        self.closure_lab_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.closure_lab_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.notebook.add(frame, text='Closure Lab')
        self.set_text(self.closure_lab_output, 'Closure Lab v7.0 is ready. Open a project, then run One-Click Closure Pass.')
    ...
    def _closure_lab_show_result(self, result, status: str):
        self.set_text(self.closure_lab_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.closure_lab_frame)
        except Exception:
            pass
        try:
            self.reload_project()
    ...
    def closure_lab_one_click_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(run_closure_lab_pass(root), 'Closure Lab pass complete')
            messagebox.showinfo('Closure Lab complete', 'Closure Lab pass completed. Review closure_lab/CLOSURE_LAB_START_HERE.md.')
        except Exception as exc:
            messagebox.showerror('Closure Lab failed', str(exc))
    ...
    def closure_lab_dashboard_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(write_closure_dashboard(root), 'Closure Lab dashboard written')
        except Exception as exc:
            messagebox.showerror('Closure dashboard failed', str(exc))
    ...
    def closure_lab_offline_runtime_ui(self):
        ...
        try:
            self._closure_lab_show_result(build_offline_runtime_simulation(root, start_state=start, max_ticks=ticks), 'Offline runtime simulation written')
        except Exception as exc:
    ...
    def closure_lab_engine_adapter_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(write_engine_adapter_suite(root), 'Engine adapter suite written')
        except Exception as exc:
            messagebox.showerror('Engine adapter suite failed', str(exc))
    ...
    def closure_lab_validate_engine_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(validate_engine_adapter_suite(root), 'Engine adapter validation written')
        except Exception as exc:
            messagebox.showerror('Engine adapter validation failed', str(exc))
    ...
    def closure_lab_dry_engine_ui(self):
        ...
        try:
            self._closure_lab_show_result(run_engine_adapter_suite(root, profile_name=profile, dry_run=True), 'Engine adapter dry-run complete')
        except Exception as exc:
    ...
    def closure_lab_run_engine_ui(self):
        ...
        try:
            self._closure_lab_show_result(run_engine_adapter_suite(root, profile_name=profile, dry_run=False, timeout_seconds=timeout), 'Engine adapter run complete')
        except Exception as exc:
    ...
    def closure_lab_sff2_corpus_ui(self):
        ...
        if messagebox.askyesno('SFF2 corpus', 'Choose a folder of .sff files? Choose No to use closure_lab/sff2_corpus/drop_sff_files_here.'):
            ...
            if chosen:
                folder = Path(chosen)
        try:
            self._closure_lab_show_result(build_sff2_corpus_validator(root, folder), 'SFF2 corpus validator written')
        except Exception as exc:
    ...
    def closure_lab_sff2_triage_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(write_sff2_unknown_triage(root), 'Unknown SFF2 triage written')
        except Exception as exc:
            messagebox.showerror('Unknown SFF2 triage failed', str(exc))
    ...
    def closure_lab_export_commit_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(export_direct_mutation_commit_sheet(root), 'Mutation commit sheet exported')
        except Exception as exc:
            messagebox.showerror('Mutation commit sheet failed', str(exc))

    def closure_lab_apply_commit_ui(self):
        root = self._closure_lab_root()
        if not root:
            return
        if not messagebox.askyesno('Apply mutation commit sheet', 'Apply enabled commit rows? This can overwrite project SFF/SND targets, but writes a rollback snapshot first.'):
            return
        try:
            self._closure_lab_show_result(apply_direct_mutation_commit_sheet(root), 'Mutation commit sheet applied')
        except Exception as exc:
            messagebox.showerror('Apply mutation commit sheet failed', str(exc))

    def closure_lab_restore_commit_ui(self):
        root = self._closure_lab_root()
        if not root:
            return
        if not messagebox.askyesno('Restore last mutation snapshot', 'Restore the last binary mutation snapshot?'):
            return
        try:
            self._closure_lab_show_result(restore_last_mutation_snapshot(root), 'Last mutation snapshot restored')
        except Exception as exc:
            messagebox.showerror('Restore mutation snapshot failed', str(exc))
    ...
    def closure_lab_tuning_sheet_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(export_gameplay_tuning_sheet(root), 'Gameplay tuning sheet exported')
        except Exception as exc:
            messagebox.showerror('Gameplay tuning sheet failed', str(exc))
    ...
    def closure_lab_apply_tuning_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(apply_gameplay_tuning_sheet(root), 'Gameplay tuning sheet applied')
        except Exception as exc:
            messagebox.showerror('Apply gameplay tuning failed', str(exc))
    ...
    def closure_lab_evidence_contract_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(write_report_evidence_contract(root), 'Report evidence contract written')
        except Exception as exc:
            messagebox.showerror('Report evidence contract failed', str(exc))
    ...
    def closure_lab_bundle_ui(self):
        ...
        if not root:
            return
        try:
            self._closure_lab_show_result(build_closure_lab_bundle(root), 'Closure Lab bundle built')
        except Exception as exc:
            messagebox.showerror('Closure Lab bundle failed', str(exc))

    # Evidence Core v7.0 — external engine evidence, corpus validation, verified candidate promotion, and runtime tuning.
    def _build_evidence_core_tab(self):
        frame = ttk.Frame(self.notebook)
        self.evidence_core_frame = frame
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)
        ...
        self.evidence_core_output = tk.Text(frame, wrap='word', font=('Consolas', 10), state='disabled')
        self.evidence_core_output.grid(row=2, column=0, sticky='nsew', padx=6, pady=(0, 6))
        self.notebook.add(frame, text='Evidence Core')
        self.set_text(self.evidence_core_output, 'Evidence Core v7.0 is ready. Open a project, then run One-Click Evidence Core Pass or configure your local engine path.')
    ...
    def _evidence_core_show_result(self, result, status: str):
        self.set_text(self.evidence_core_output, result.to_text() if hasattr(result, 'to_text') else str(result))
        self.status_var.set(status)
        try:
            self.notebook.select(self.evidence_core_frame)
        except Exception:
            pass
        try:
            self.reload_project()
    ...
    def evidence_core_one_click_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.run_evidence_core_pass(root), 'Evidence Core pass complete')
            messagebox.showinfo('Evidence Core complete', 'Evidence Core pass completed. Review evidence_core/EVIDENCE_CORE_START_HERE.md.')
        except Exception as exc:
            messagebox.showerror('Evidence Core failed', str(exc))
    ...
    def evidence_core_dashboard_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.write_evidence_core_dashboard(root), 'Evidence Core dashboard written')
        except Exception as exc:
            messagebox.showerror('Evidence Core dashboard failed', str(exc))
    ...
    def evidence_core_configure_engine_ui(self):
        ...
        try:
            self._evidence_core_show_result(ev_core.write_engine_profile(root, engine_exe=Path(exe), game_root=Path(exe).parent, default_stage=default_stage, opponent=opponent), 'Evidence Core engine profile written')
        except Exception as exc:
    ...
    def evidence_core_probe_engine_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.probe_engine_profile(root), 'Evidence Core engine probe written')
        except Exception as exc:
            messagebox.showerror('Evidence Core engine probe failed', str(exc))
    ...
    def evidence_core_launch_matrix_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.build_engine_launch_matrix(root), 'Evidence Core launch matrix written')
        except Exception as exc:
            messagebox.showerror('Evidence Core launch matrix failed', str(exc))
    ...
    def evidence_core_dry_run_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.run_engine_validation(root, dry_run=True), 'Evidence Core dry-run validation written')
        except Exception as exc:
            messagebox.showerror('Evidence Core dry-run failed', str(exc))
    ...
    def evidence_core_authority_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.write_evidence_authority_report(root), 'Evidence authority report written')
        except Exception as exc:
            messagebox.showerror('Evidence authority report failed', str(exc))
    ...
    def evidence_core_corpus_run_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.run_sff2_corpus_validation(root), 'SFF2 corpus validation written')
        except Exception as exc:
            messagebox.showerror('SFF2 corpus validation failed', str(exc))
    ...
    def evidence_core_candidate_sheet_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.export_verified_candidate_install_sheet(root), 'Verified candidate install sheet written')
        except Exception as exc:
            messagebox.showerror('Verified candidate sheet failed', str(exc))

    def evidence_core_apply_candidate_ui(self):
        root = self._evidence_core_root()
        if not root:
            return
        if not messagebox.askyesno('Apply verified candidate install sheet', 'Apply enabled rows from evidence_core/candidate_promotion/verified_candidate_install_sheet.csv? Current targets will be backed up first.'):
            return
        try:
            self._evidence_core_show_result(ev_core.apply_verified_candidate_install_sheet(root), 'Verified candidate install sheet applied')
        except Exception as exc:
            messagebox.showerror('Apply verified candidate sheet failed', str(exc))
    ...
    def evidence_core_tuning_sheet_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.export_runtime_tuning_sheet(root), 'Runtime tuning sheet written')
        except Exception as exc:
            messagebox.showerror('Runtime tuning sheet failed', str(exc))

    def evidence_core_apply_tuning_ui(self):
        root = self._evidence_core_root()
        if not root:
            return
        if not messagebox.askyesno('Apply runtime tuning sheet', 'Apply enabled rows from evidence_core/runtime_tuning/runtime_tuning_sheet.csv? Source files will be backed up first.'):
            return
        try:
            self._evidence_core_show_result(ev_core.apply_runtime_tuning_sheet(root), 'Runtime tuning sheet applied')
        except Exception as exc:
            messagebox.showerror('Apply runtime tuning failed', str(exc))
    ...
    def evidence_core_playtest_closure_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.write_playtest_closure_report(root), 'Playtest closure report written')
        except Exception as exc:
            messagebox.showerror('Playtest closure failed', str(exc))
    ...
    def evidence_core_bundle_ui(self):
        ...
        if not root:
            return
        try:
            self._evidence_core_show_result(ev_core.build_evidence_core_bundle(root), 'Evidence Core bundle built')
        except Exception as exc:
            messagebox.showerror('Evidence Core bundle failed', str(exc))
